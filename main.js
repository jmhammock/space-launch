...
var $author$project$Main$Launch = F6(
	function (id, url, name, status, image, windowStart) {
		return {id: id, image: image, name: name, status: status, url: url, windowStart: windowStart};
	});
...
var $author$project$Main$viewButtons = function (model) {
	switch (model.$) {
		...
		default:
			var response = model.a;
			var _v1 = _Utils_Tuple2(response.previous, response.next);
			if (_v1.a.$ === 'Just') {
				if (_v1.b.$ === 'Just') {
					return A2(
						$elm$html$Html$div,
...
var $author$project$Main$viewLaunch = function (launch) {
	var windowStart = $rtfeldman$elm_iso8601_date_strings$Iso8601$toTime(launch.windowStart);
	return A2(
		...
		_List_fromArray(
			[
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										A2($elm$html$Html$Attributes$style, 'background-image', 'url(' + (launch.image + ')')),
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Status: ' + launch.status.name)
					])),
				A2(
				$elm$html$Html$div,
				_List_Nil,
	...
	switch (model.$) {
		...
		default:
			...
			return A2(
				$elm$html$Html$div,
				_List_Nil,
				A2($elm$core$List$map, $author$project$Main$viewLaunch, response.results));
	}
};
